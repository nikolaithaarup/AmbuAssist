...
function outcomeFromSelections(selections: Selection[]): OutcomeId | null {
  // Walk through the decision tree from step 1.
  let current: StepId | OutcomeId = "penetrating";

  // We’ll consult selections by stepId (latest wins, though it’s linear anyway).
  const selMap = new Map<StepId, AnswerKey>();
  selections.forEach((s) => selMap.set(s.stepId, s.answer));

  // Prevent infinite loops; there are only a few nodes.
  for (let i = 0; i < 10; i++) {
    if (
      current === "none" ||
      current === "spinal" ||
...
export default function SpinalTraumaFlow() {
  ...
  const currentStepId: StepId | null = useMemo(() => {
    // Determine which step we’re currently on by replaying answers.
    let current: StepId | OutcomeId = "penetrating";
  ...
  function answer(stepId: StepId, a: AnswerKey) {
    setSelections((prev) => {
      // Keep answers up to stepId (linear wizard behavior)
    ...
    <Background>
      <Screen>
        ...
        <ScrollView contentContainerStyle={{ gap: 12, paddingBottom: 24 }}>
          {/* Collapsible “definition” / help box */}
          <Pressable
          ...
          {/* Tiny disclaimer (optional but sensible) */}
